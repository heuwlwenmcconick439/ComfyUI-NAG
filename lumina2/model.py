    ...
    def forward(
        ...
        **kwargs,
        ...
        if apply_nag:
            ...
            # Batch-doubled mode: concatenate pos+neg for all inputs
            ...
            # Patch Attention Modules via class swizzling
            for name, module in self.named_modules():
                if isinstance(module, JointAttention) and hasattr(module, "qkv"):
                    ...
                    module._nag_origin_bsz = origin_bsz
        ...
        try:
            ...
            # Restore Original Classes (Always run, even on crash)
            if apply_nag:
                for mod, original_class in modules_patched:
                    ...
                    if hasattr(mod, '_nag_origin_bsz'):
                        delattr(mod, '_nag_origin_bsz')
                modules_patched.clear()

        # Post-process: take only first half (guided positive)
